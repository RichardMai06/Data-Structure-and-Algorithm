...
List *createList(NODE *p_node); // done
bool addHead(List *&L, int data); //done
bool addTail(List *&L, int data); // done
bool removeHead(List *&L); // done
void removeTail(List *&L); // done
void removeAll(List *&L); // done
void removeBefore(List *&L, int val); // done
void removeAfter(List *&L, int val); // done
bool addPos(List *&L, int data, int pos); // done
void removePos(List *&L, int data, int pos); // done
bool addBefore(List *&L, int data, int val); //done
bool addAfter(List *&L, int data, int val);
void printList(List *L); //done
int countElements(List *L); //done
List *reverseList(List *L); // done
void removeDuplicate(List *&L); // done
bool removeElement(List *&L, int key); //done
...
bool removeHead(List *&L)
{
    ...
    NODE *current = L->p_head;
    current->p_next->p_previous = nullptr;
...
void removeTail(List *&L)
{
    ...
    NODE *current = L->p_tail;
    current->p_previous->p_next = nullptr;
    L->p_tail = current->p_previous;
    delete current;
} 
...
// Remove a node before a given value in the list if exist, otherwise do nothing
void removeBefore(List *&L, int val)
{
    if (L->p_head == nullptr || L->p_head == L->p_tail || L->p_head->key == val)
    {
        return;
    }
    NODE *current = L->p_head;
    ...
    {
        if (current->p_next->key == val && current == L->p_head)  
        {
            removeHead(L);
            return;
        }
        else if (current->p_next->key == val)
        {
...
{
    if (L->p_head == nullptr || L->p_head == L->p_tail || L->p_tail->key == val)
    {
        return;
    }
    ...
    {
        if (current->p_previous->key == val && current == L->p_tail)
        {
            removeTail(L);
            return;
        }
        else if (current->p_previous->key == val)
        {
            current->p_previous->p_next = current->p_next;
            current->p_next->p_previous = current->p_previous;
            delete current;
...
bool addPos(List *&L, int data, int pos)
{
    ...
    else if (pos > size / 2)
    {
        ...
        new_node->p_previous = current->p_previous;
        new_node->p_next = current;
        current->p_previous->p_next = new_node;
        current->p_previous = new_node;
        return true;
    }
    else
    {
        ...
        new_node->p_previous = current;
        new_node->p_next = current->p_next;
        current->p_next->p_previous = new_node;
...
// Insert an integer before a value of the given list
...
List *reverseList(List *L)
{
    ...
    NODE *temp = nullptr, *current = L->p_head;
    while (current != nullptr)
    {
        temp = current->p_next;
        current->p_next = current->p_previous;
...
void removeDuplicate(List *&L)
{
    ...
    while (current != nullptr)
    {
        ...
        while (temp != nullptr)
        {
            NODE *next = temp->p_next;
            if (current->key == temp->key)
            {
                if (temp->p_previous != nullptr)
                {
                    temp->p_previous->p_next = temp->p_next;
                }
                if (temp->p_next != nullptr)
                {
                    temp->p_next->p_previous = temp->p_previous;
                }
                if (temp == L->p_tail)
                {
                    L->p_tail = temp->p_previous;
                }
                delete temp;
            }
            temp = next;
