...
bool addAfter(List *&L, int data, int val); // done
...
void removeTail(List *&L)
{
    ...
    current->p_next = L->p_tail->p_next;
    delete L->p_tail;
    L->p_tail = current;
}
...
void removeBefore(List *&L, int val)
{
    ...
    NODE *slow = L->p_head, *fast = slow->p_next->p_next;
    while (fast != nullptr)
    {
        if (fast->key == val)
        {
...
{
    if (L->p_head == nullptr || L->p_head->p_next == nullptr || L->p_tail->key == val)
    {
        return;
    }
    NODE *slow = L->p_head, *fast = slow->p_next;
    while (fast != nullptr)
    {
        if (slow->key == val)
        {
...
bool addBefore(List *&L, int data, int val)
{
    ...
    while (fast != nullptr)
    {
        if (fast->key == val)
        {
            ...
            new_node->p_next = fast;
            slow->p_next = new_node;
            return true;
        }
        else
        {
...
List *reverseList(List *L)
{
    ...
    while (current != nullptr)
    {
        addHead(temp, current->key);
    }
    L->p_head = temp->p_head;
    L->p_tail = temp->p_tail;
    delete temp;   
    return L;
}
...
void removeDuplicate(List *&L)
{
    ...
    while (current != nullptr)
    {
        NODE *slow = current, *fast = slow->p_next;
        while (fast != nullptr)
        {
...
bool removeElement(List *&L, int key)
{
    ...
    NODE *slow = L->p_head, *fast = slow->p_next;
    while (fast->p_next != nullptr)
    {
        if (fast->key == key)
        {
            removeAfter(L, slow->key);
            return true;
        }
        else
        {
